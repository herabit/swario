...
impl U64x2 {
    ...
    /// Performs an unchecked right shift on every [`u64`] lane by `n` bits.
    ///
    /// # Safety
    ///
    ...
    pub const unsafe fn unchecked_shr(self, n: u32) -> U64x2 {
        ...
        //       right shift methods on [`u64`]s accept a [`u32`] as an argument.
        //
        ...
        //
        //         One such example is the following mask calculation, if the compiler knows that `u64::BITS - n` never
        //         will over/underflow, it permits the omission of Rust's automatically inserted debug checks for the subtraction
        //         `u64::BITS - 1 - n`. In release builds, Rust will always default to wrapping subtraction, but due to this
        //         utilization of LLVM's `assume` intrinsic (which `assert_unchecked` lowers to), we're telling the compiler that,
        //         it is impossible for `n >= u64::BITS` to ever be true.
        //
