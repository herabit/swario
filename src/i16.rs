...
impl I16x2 {
    ...
    /// Performs an unchecked right shift on every [`i16`] lane by `n` bits.
    ///
    /// # Safety
    ///
    ...
    pub const unsafe fn unchecked_shr(self, n: u32) -> I16x2 {
        ...
        //       right shift methods on [`i16`]s accept a [`u32`] as an argument.
        //
        ...
        //
        //         One such example is the following mask calculation, if the compiler knows that `i16::BITS - n` never
        //         will over/underflow, it permits the omission of Rust's automatically inserted debug checks for the subtraction
        //         `i16::BITS - 1 - n`. In release builds, Rust will always default to wrapping subtraction, but due to this
        //         utilization of LLVM's `assume` intrinsic (which `assert_unchecked` lowers to), we're telling the compiler that,
        //         it is impossible for `n >= i16::BITS` to ever be true.
        //
        ...
        //     sign_mask.wrapping_sub(sign_mask >> i16::BITS - 1)
        // );
        //
        // let shifted = ((self.0 ^ neg_mask) >> n) ^ neg_mask;
        //
        // return I16x2(shifted & mask);
        // ```
